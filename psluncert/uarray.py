...
class ArrayFunc(uncertainty.InputFunc):
    ...
    '''
    def __init__(self, function=None, arr=None, name=None, desc='', seed=None):
...
class ArrayThresh(ArrayFunc):
    ...
    '''
    def __init__(self, arr=None, name=None, desc='', thresh=0, edge='first', seed=None):
        self.thresh = thresh
        self.edge = edge
        super(ArrayThresh, self).__init__(function=self._function, arr=arr, name=name, desc=desc, seed=seed)
    ...
    def mean(self):
        ...
        if 'lsq' not in self.outputs:
            self.calc_LSQ()
        return self.outputs['lsq'].mean

    def stdunc(self):
        ''' Get standard uncertainty value '''
        if 'lsq' not in self.outputs:
            self.calc_LSQ()
        return self.outputs['lsq'].uncert
