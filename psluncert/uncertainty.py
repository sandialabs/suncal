...
class InputVar(object):
    ''' Input variable class. Contains a mean/expected value and a list of uncertainty components.
        ...
        '''
    def __init__(self, name, nom=1, desc=''):
        self.name = name
        self.nom = nom
        self.desc = desc
        self.uncerts = []
    ...
    def __repr__(self):
        return '<InputVar> {} = {}'.format(self.name, self.nom)
    ...
    def stdunc(self):
        ...
        '''
        ss = np.nansum([u.var() for u in self.uncerts])
        return np.sqrt(ss)
    ...
    def degf(self):
        ...
        num2 = np.nansum([u.var() for u in self.uncerts])
        denom = np.nansum([u.var()**2 / u.degf for u in self.uncerts])
        return np.float64(num2)**2 / denom  # Use numpy float so div/0 returns inf.

    def add_comp(self, name, dist='normal', degf=np.inf, desc='', **args):
        ''' Add uncertainty component to this variable. If uncertainty name
            ...
            dist: string or scipy.stats.rv_continuous
                ...
                name of distribution in psluncert.customdists, or an instance
        ...
        if name in unames:
            ...
            self.uncerts[idx] = InputUncert(name, dist=dist, degf=degf, desc=desc, nom=self.nom, **args)
        else:
            self.uncerts.append(InputUncert(name, dist=dist, degf=degf, desc=desc, nom=self.nom, **args))
        self.normsamples = None
        self.sampledvalues = None
    ...
    def sample(self, samples=1000000):
        ...
        self.sampledvalues = np.ones(samples) * self.nom
        for u in self.uncerts:
...
class InputUncert(object):
    ...
    '''
    def __init__(self, name, nom=0, dist='normal', degf=np.inf, desc='', **args):
        self.name = name
        self.distname = dist
        self.degf = degf
        self.nom = nom
    ...
    def sample(self, samples=1000000, inc_nom=True):
        ...
        if inc_nom:
            self.sampledvalues = self.sampledvalues + self.nom
        return self.sampledvalues
    ...
    def std(self):
        ...
        return self.distfunc.std()

    def var(self):
        ''' Return the variance of the distribution function'''
        return self.distfunc.var()
...
class InputFunc(object):
    ''' Class for an input function. Can be used as an InputVar.
        ...
        '''
    def __init__(self, function=None, variables=None, name=None, desc=''):
    ...
    def _numgradient(self):
        ...
        means = self.get_basemeans()  # Unordered!!
        basenames = self.get_basenames()
        basefunc = self.get_basefunc()
        baseuncerts = self.get_baseuncerts()
        grad = []
        for name in basenames:
            d1 = means.copy()
            dx = np.float64(baseuncerts[name] / 1E6)  # Float64 won't have div/0 errors, will go to inf
            if dx == 0:
                dx = 1E-6
            d1[name] = d1[name] + dx
            d2 = means.copy()
            d2[name] = d2[name] - dx
            grad.append((uparser.callf(basefunc, d1) - uparser.callf(basefunc, d2))/(2*dx))
    ...
    def _symgradient(self):
        ...
        for x in basenames:
            try:
                grad.append(sympy.Derivative(basefunc, sympy.Symbol(x), evaluate=True).subs(means).evalf())
            except OverflowError:
                grad.append(sympy.nan)
    ...
    def get_latex(self):
        ...
        if self.name == '':
            ...
            return sympy.latex(sympy.sympify(self.name))
    ...
    def calc_GUM(self, correlation=None, calc_sym=True):
        ...
        u = np.array([i.stdunc() for i in basevars])
        # REF note 3 in 5.2.2 GUM. Formula uses correlation, not covariance.
        try:
    ...
    def calc_MC(self, samples=1000000, sensitivity=True):
        ...
        if sensitivity:
            ...
            for x in xvals.keys():
                xvals[x] = np.full(samples, xvals[x])
            ...
            for v in self.get_basevars():  # Hold all inputs fixed but one, compare output stddev.
                ...
                sens.append(ustd/v.stdunc())
                props.append((ustd/stdY)**2 * 100)
    ...
    def calc_SYM(self, correlation=None, uncert=None, degf=None):
        ...
        for p in partials:
            try:
                partials_solved.append(p.evalf(subs=Xmeans))
...
class UncertCalc(object):
    ''' Main Uncertainty Calculator Class.
        ...
        '''
    def __init__(self, function=None, inputs=None, samples=1000000, seed=None, name='uncertainty'):
        ...
        if isinstance(function, list) or isinstance(function, tuple):
            [self.set_function(f) for f in function]
        ...
        if inputs is not None:
            for inpt in inputs:
                ...
                iname = inpt.get('name', '')
                self.set_input(iname, nom=inpt.get('nom', 0), desc=inpt.get('desc', ''))
                for udict in uncerts:
                    self.set_uncert(var=iname, **udict)

    def set_function(self, func, idx=None, name=None, desc='', show=True, kwnames=None):
        ...
        if not hasattr(func, 'calc_GUM'):  # isinstance(func, InputFunc)  # Doesn't seem to work across uarray module
            func = InputFunc(func, variables=self.variables, name=name, desc=desc)  # self.variables list passed as object
    ...
    def set_input(self, inpt, nom=1, desc='', **kwargs):
        ...
        if idx is None:  # Not an existing InputVar. Add it
            if not isinstance(inpt, InputVar):
                inpt = InputVar(name=inpt, nom=nom, desc=desc)
    ...
    def set_uncert(self, var, name=None, dist='normal', degf=np.inf, desc='', **args):
        ...
        self.variables[idx].add_comp(name=name, dist=dist, degf=degf, desc=desc, **args)
    ...
    def add_required_inputs(self):
        ...
        [self.set_input(i) for i in inputs if i not in [x.name for x in self.variables]]  # Add new variables
    ...
    def get_contour(self, func1, func2):
        ...
        >>> plt.contour(x, y, p)
        '''
        m0 = self.functions[func1].out.gum.mean[0]
        m1 = self.functions[func2].out.gum.mean[0]
        u0 = self.functions[func1].out.gum.uncert[0]
        u1 = self.functions[func2].out.gum.uncert[0]
        ...
        baseinputs = self.get_baseinputs()
        s1 = np.zeros(len(baseinputs))
        s2 = np.zeros(len(baseinputs))
        ...
        S = np.diag([i.stdunc() for i in self.get_baseinputs()])
    ...
    def get_config(self):
        ...
        for f in self.functions:
            fdict = {'name': f.name, 'expr': str(f.function), 'desc': f.desc}
            d['functions'].append(fdict)

        for inpt in self.get_baseinputs():
            idict = {'name': inpt.name, 'mean': inpt.nom, 'desc': inpt.desc}
            ulist = []
            if len(inpt.uncerts) > 0:
                for unc in inpt.uncerts:
                    udict = {'name': unc.name, 'desc': unc.desc, 'degf': unc.degf}
    ...
    def save_samples(self, fname, fmt='csv', inputs=True, outputs=True):
        ...
        if inputs:
            ...
            hdr = [v.name for v in variables]
            hdr.extend([str(f) for f in self.functions])
            csv = np.stack([v.sampledvalues for v in variables], axis=1)
        if outputs:
            out = np.array([self.out.foutputs[f].mc.samples[:, 0] for f in range(len(self.functions))]).T
            if inputs:
    ...
    def from_config(cls, config):
        ...
        if 'functions' in config:
            for func in config['functions']:
                if type(func) == dict:
                    try:
                        u.set_function(func.get('expr'), name=func.get('name'), desc=func.get('desc', ''))
                    except ValueError:
        ...
        if 'inputs' in config:
            for inpt in config['inputs']:
                if type(inpt) == dict:
                    newinpt = u.set_input(inpt.get('name', ''), nom=get_float(inpt, 'mean'), desc=inpt.get('desc', ''))
                    if 'uncerts' in inpt:
                        for unc in inpt['uncerts']:
